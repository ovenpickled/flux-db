...
const size_t k_max_msg = 32 << 20;      // most likely bigger than the kernel buffer
...
// KV pairs for the top level hashtable
struct Entry {
...
static void heap_delete(std::vector<HeapItem> &a, size_t pos) {
  // swap the erased item with them last item
  a[pos] = a.back();
  a.pop_back();
...
static void entry_set_ttl(Entry *ent, int64_t ttl_ms) {
  if (ttl_ms < 0 && ent -> heap_idx != (size_t)-1) {
    ...
    // add pf update the heap data structure
...
static void handle_read(Conn *conn) {
	...
	while (try_one_request(conn)) {}        // looped
...
static int32_t next_timer_ms() {
  ...
  // idle timer's using a linked list
...
int main() {
  ...
  aof_open("aof.log");
	...
	while (true) {
		...
		// rest will be connection sockets
